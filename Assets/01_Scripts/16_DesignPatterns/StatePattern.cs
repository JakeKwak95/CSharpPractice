...
public class StatePattern : MonoBehaviour
{
	...
	{
		// 초기 상태를 순찰 상태로 설정
		ChangeState(PlayerState.Patrolling);
	}
	...
	void ChangeState(PlayerState newState)
	{
		...
		StartCoroutine("Co" + currentState.ToString());
	}
	...
	IEnumerator CoPatrolling()
	{
		...
		while (true)
		{
			...
			if (Vector3.Distance(transform.position, chaseTarget.position) <= chaseRange)
			{
				...
				ChangeState(PlayerState.Chasing);
				yield break;
			}
			// 타겟 포인트에 도달할 때까지 이동
			else if (!Mathf.Approximately(0f, Vector3.Distance(transform.position, patrolPoints[currentPatrolIndex].position)))
			{
	...
	IEnumerator CoChasing()
	{
		while (true)
		{
			...
			else if (Vector3.Distance(transform.position, chaseTarget.position) <= attackRange)
			{
	...
	{
		while (Vector3.Distance(transform.position, chaseTarget.position) <= attackRange)
		{
